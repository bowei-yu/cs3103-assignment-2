...
def getCompletedFilename(filename):
    ...
    N = (100 * response_time) * (active_connections)

    # Update four_tuple in server_list and sort
    server_list[position][0] = N
    ...
    # Adjust Real Weight (rW) = Number of Active Connections x (10000 / Set Weight (sW))
    for idx, val in enumerate(server_list):
        if server_name in val:
            server_list[idx][0] = N * (10000 / ((len(server_list) - (idx + 1) + 1) * 10000))
            break
    server_list.sort()
