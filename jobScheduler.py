...
def getCompletedFilename(filename):
    ...
    weight = (10000 / ((len(server_list) - (idx + 1) + 1)))
    weighted_response_time = N * weight
    weighted_connection_time = active_connections * weight                

    # Update four_tuple in server_list and sort
    server_list[position][0] = (1/3 * N) + (1/3 * weighted_response_time) + (1/3 * weighted_connection_time)
    ...
    for idx, val in enumerate(server_list):
        if server_name in val:
            ...
            if response_time != 0:
                ...
                weighted_response_time = N * weight
                weighted_connection_time = active_connections * weight
                server_list[idx][0] = (1/3 * N) + (1/3 * weighted_response_time) + (1/3 * weighted_connection_time)
            break
    server_list.sort()
